...
class MainWindow(QWidget):
    ...
    def setup_info_erlang_view(self):
        ...
        theory_text = (
            ...
            "Oblicza prawdopodobieństwo blokady prób połączeń w systemie z N kanałami i natężeniem ruchu A.<br><br>"
            "<b>Parametry:</b><br>"
            "A - natężenie ruchu w Erlangach (suma czasów trwania połączeń w jednostce czasu)<br>"
            "N - liczba dostępnych kanałów obsługi<br>"
            "B - docelowe prawdopodobieństwo blokady (wynik wzoru)"
        )
        ...
        text_widget.setGeometry(50, 230, 700, 200)
        ...
        newFont = QFont(self.fontSmall)
        newFont.setPointSize(15)
        buttonBack.setFont(newFont)
        buttonBack.setGeometry(580, 540, 180, 50)
    ...
    def setup_info_enset_view(self):
        ...
        theory_text = (
            "<b>Wzór Engseta:</b><br>"
            "Modeluje prawdopodobieństwo blokady w systemie z ograniczoną liczbą źródeł ruchu.<br><br>"
            "<b>Parametry:</b><br>"
            "a - liczba użytkowników (źródeł ruchu)<br>"
            "A - natężenie ruchu generowane przez jedno źródło (w Erlangach)<br>"
            "N - liczba kanałów obsługi<br>"
            "B - docelowe prawdopodobieństwo blokady (wynik wzoru)"
        )
    ...
    def enset_chooser(self):
        ...
        if self.option1_A.isChecked():     # obliczamy A
            ...
            if self.plotParamB_E.isChecked():
                ...
                draw_chart(Bs, As, "Prawdopodobieństwo blokady B", "Natężenie ruchu A")

            elif self.plotParamN_E.isChecked():

                Ns = list(range(1, value_N + 1))
                As = [Engset.engset_A(value_Num, n, value_B) for n in Ns]
                draw_chart(Ns, As, "Liczba kanałów N", "Natężenie ruchu A")
        ...
        # walidacja A, N, Num
            ...
            if self.plotParamN_E.isChecked():
                ...
                draw_chart(Ns, As, "Liczba kanałów N", "Prawdopodobieństwo blokady B")
                ...
                draw_chart(As1, As, "Natężenie ruchu A", "Prawdopodobieństwo blokady B")
            ...
            if self.plotParamB_E.isChecked(): 
                ...
                draw_chart(Bs_filtered, Ns_filtered, "Prawdopodobieństwo blokady B", "Liczba kanałów N")
                ...
                draw_chart(As_filtered, Ns_filtered, "Natężenie ruchu A", "Liczba kanałów N")
    ...
    def erlang_chooser(self):
        ...
        if self.option1_AEL.isChecked():  # liczymy A, mamy B i N
            ...
            if self.plotParamB.isChecked():
                ...
                draw_chart(Bs, As, "Prawdopodobieństwo blokady B", "Liczba kanałów N")

            elif self.plotParamA.isChecked():
                # wariacja po B – wykres Erlang A = f(B) przy stałym N
                # dobieramy 50 punktów w przedziale [0.001, 0.999]
                As1 = list(range(1, int(value_A) + 1))
                As = [erlang.erlang_b(value_A, b) for b in As1]
                draw_chart(As1, As, "Natężenie ruchu A", "Liczba kanałów N")
        else:
            self.result_window.add_entry("Błąd: wybierz jedną opcję", 0)
